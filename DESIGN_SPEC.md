...
4. **Smart suggestion banner component** — NEXT. Plan: put pure logic in `lib/suggestions.js` so step 5 analytics can reuse the "last trained per muscle", "sessions per muscle per week", and "push/pull tally" calculations. `SuggestionBanner` component consumes that helper and renders above "Recent activity" on the Log tab. Implements Rules 1-5 from the "Smart Suggestion Engine" section above (recovery enforcement, weekly frequency target, 2-week rotation, compound preference, push/pull balance).
