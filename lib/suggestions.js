...
//
// Variety amendments (2026-06-09): never-done novelty bonus (+12) outranks
// stale rotation (+5); picks are sampled 60/25/15 from the top 3 scores using
// a date-seeded RNG (stable within a day, rotates daily); Isolation is exempt
// from the pattern-coverage bonus (catch-all bucket starved isolation moves);
// one exercise per primary muscle per session (soft — relaxes if pool empties).
//
// Public API:
...
function gapScore(libRow, ctx) {
  ...
  if (last) {
    ...
    // never done — strong novelty boost. Must beat the stale-rotation +5,
    // otherwise never-logged exercises can never out-rank old favorites.
    score += 12;
  }
  ...
  // Isolation is excluded — it's the catch-all bucket (~half the library), so
  // it always has hits and its exercises would never earn the bonus, which
  // starved isolation variants (laterals, pushdowns) out of suggestions.
  const bucket = normalizePattern(libRow.movement_pattern);
  if (bucket !== PATTERNS.ISOLATION) {
...
/**
 * Deterministic PRNG seeded from a string (mulberry32 over a string hash).
 * Seeding by the session date keeps suggestions stable across re-renders
 * within a day, while still rotating picks day to day.
 */
...
/**
 * Pick one of the top-3 scored candidates with weights 60/25/15 (normalized
 * when fewer than 3 exist). Breaks the old always-argmax behavior that made
 * the same winners repeat forever.
 */
...
export function buildSession(exercises, library, today, count = DEFAULT_SESSION_COUNT) {
  ...
  const refDay = parseDay(today) ?? new Date();
  const rng = seededRng(
    `${refDay.getFullYear()}-${refDay.getMonth() + 1}-${refDay.getDate()}`
  );

  function pickBest(predicate, role, rationaleBuilder) {
    const buildPool = (excludeUsedMuscles) => {
      const pool = [];
      for (const l of eligible) {
        if (used.has(l.name)) continue;
        if (excludeUsedMuscles && l.primary_muscle && usedMuscles.has(l.primary_muscle)) continue;
    ...
    // Prefer a primary muscle not already in the session; relax if that
    // empties the pool (small type pools, recovery blocks).
    let pool = buildPool(true);
    if (pool.length === 0) pool = buildPool(false);
    if (pool.length === 0) return null;

    pool.sort((a, b) => b.score - a.score);
    const top = weightedPick(pool, rng);
    used.add(top.lib.name);
    if (top.lib.primary_muscle) usedMuscles.add(top.lib.primary_muscle);
    session.push({
      exercise: top.lib,
      pattern: top.pattern,
      role,
